<?php 
	...
	if(isset($_POST['update-prof'])) {
		...
		$sql="UPDATE Users SET firstname='".$fn."' WHERE user_id=$id";
		mysql_query($sql);
		$sql1="UPDATE Users SET lastname='".$ln."' WHERE user_id=$id";
		mysql_query($sql1);
	...
	if(isset($_POST['change-pass'])) {
		...
		if ($row['password']==$old) {
			if ($new==$conf) {
				mysql_query("UPDATE Users SET password='".$new."' WHERE user_id=$id");
